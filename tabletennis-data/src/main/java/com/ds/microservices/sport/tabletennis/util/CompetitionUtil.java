...
import com.ds.microservices.sport.tabletennis.model.CompetitionPlayerPK;
...
//		logger.info("Prop 1 " + competition.getPropertiesMap().get("NUMBER_OF_PLAYERS"));
		...
		if (!autogeneratedPlayers) {
			completed = ((competition.getCompetitionPlayers() == null) ? 0 : competition.getCompetitionPlayers().size()) == Long.parseLong(competition.getPropertiesMap().get("NUMBER_OF_PLAYERS").getValue());
...
//		completed = (competition.getPlayers().size() == Long.parseLong(map.get("NUMBER_OF_SEEDS").getValue()))
	...
	public List<Group> createGroups(Competition competition, List<Player> candidates, boolean autogeneratedPlayers, boolean autogeneratedSeeds) {
		...
		Long number_of_seeds = Long.parseLong(competition.getPropertiesMap().get("NUMBER_OF_SEEDS").getValue());
		Long number_of_players = Long.parseLong(competition.getPropertiesMap().get("NUMBER_OF_PLAYERS").getValue());
		...
		int totalPlayers = Integer.parseInt(competition.getPropertiesMap().get("NUMBER_OF_PLAYERS").getValue());
			...
			if (autogeneratedPlayers) {
				for (Player player : candidates) {
					if (player.isActive() && missingPlayers > 0) {
						if (!competition.getCompetitionPlayersMap().containsKey(new CompetitionPlayerPK(competition.getId(), player.getId()))) {
							CompetitionPlayer competitionPlayer = new CompetitionPlayer();
							CompetitionPlayerPK pk = new CompetitionPlayerPK(competition.getId(), player.getId());
							competitionPlayer.setId(pk);
							competitionPlayer.setPlayer(player);
							competitionPlayer.setCompetition(competition);
							
							competition.getCompetitionPlayersMap().put(competitionPlayer.getId(), competitionPlayer);
...
//		sortedPlayers.sort(new PlayerComparator());
		...
		for (CompetitionPlayer competitionPlayer : competition.getCompetitionPlayers()) {
			logger.info("CompetitionPlayer " + competitionPlayer.getPlayer());
		}
	...
	public List<Game> generateGamesForGroup(Competition competition, Group group) {
		...
		for (int round = 1; round <= number_of_rounds; round++) {
			...
			for (int i = 0; i < number_of_players/2; i++) {
				...
				Game game = new Game();
				game.setPlayerHome(first.get(i).getPlayer());
				game.setPlayerAway(second.get(i).getPlayer());
